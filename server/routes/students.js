...
router.get('/:id/records', authMiddleware, async (req, res) => {
    try {
        ...
        const [studentRes, complaintsRes, csRes, violRes] = await Promise.all([
            supabase
                .from('students')
            ...
            supabase
                ...
                .select('id, violation, violation_count, shb_article_section, sanction, date_of_violation, status, created_at')
                .eq('student_id', sid)
                .order('created_at', { ascending: false })
        ]);
