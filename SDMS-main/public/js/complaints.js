...
async function renderComplaints() {
      ...
      <div class="card-header">
        <div class="toolbar">
          <div class="search-box">
            <span class="si">🔍</span>
            <input type="text" id="compSearch" placeholder="Search by subject..." />
          </div>
    ...
    document.getElementById('compSearch').addEventListener('input', debounce(e => {
        compPage = 1; loadComplaints(e.target.value);
    }, 400));
...
async function loadComplaints(search = '') {
    try {
        const data = await apiCall('GET', `/api/complaints?page=${compPage}&limit=10&search=${encodeURIComponent(search)}`);
        if (!data) return;
