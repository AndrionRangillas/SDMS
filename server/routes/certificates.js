...
router.post('/generate', authMiddleware, async (req, res) => {
    try {
        ...
        doc.fontSize(13).text('PREFECT OF STUDENT DISCIPLINE', { align: 'center' });
        doc.moveDown(0.5);
        doc.moveTo(80, doc.y).lineTo(515, doc.y).stroke();
        doc.moveDown(1.5);

        // Title
        doc.fontSize(22).font('Helvetica-Bold').text('GOOD MORAL CHARACTER CERTIFICATE', { align: 'center' });
        doc.moveDown(2);

        // Body
        doc.fontSize(12).font('Helvetica').text('TO WHOM IT MAY CONCERN:', { align: 'left' });
        doc.moveDown(1);
        doc.fontSize(12).text(
            `This is to certify that `, { continued: true }
        );
