...
async function renderViolations() {
    ...
    <div class="card">
      ...
      <div class="table-wrapper">
        <table>
          ...
          <tbody id="violTbody"><tr><td colspan="9"><div class="empty-state"><div class="spinner"></div></div></td></tr></tbody>
        </table>
      </div>
    ...
    violSearch = '';
    loadViolations();
    document.getElementById('violSearch').addEventListener('input', debounce((e) => {
        violSearch = e.target.value;
        violPage = 1;
        loadViolations();
    }, 400));
...
async function loadViolations() {
    try {
        let url = `/api/violations?page=${violPage}&limit=10`;
        if (violSearch) url += `&search=${encodeURIComponent(violSearch)}`;
        const data = await apiCall('GET', url);
        if (!data) return;
        const tbody = document.getElementById('violTbody');
        if (!data.data?.length) {
            tbody.innerHTML = `<tr><td colspan="9"><div class="empty-state"><div class="empty-icon">⚠️</div><p>No violation slips</p></div></td></tr>`;
            document.getElementById('violPagination').innerHTML = '';
            return;
        }
        tbody.innerHTML = data.data.map(v => {
            let csLink = '';
            if (v.community_service && v.community_service.length > 0) {
                const cs = v.community_service[0];
                csLink = `<div style="margin-top: 4px; font-size: 0.8rem;"><a href="#" onclick="navigateTo('community-service'); return false;" class="student-id-link">🔍 View CS Status (${cs.status})</a></div>`;
            }
            
            return `
      <tr>
        <td><strong>${v.students?.full_name || '—'}</strong><br><small style="color:var(--text-muted)"><a class="student-id-link" onclick="openStudentRecords('${v.student_id}')" title="View Records">${v.students?.student_id || ''}</a></small></td>
        ...
        <td><span class="badge badge-${(v.status || 'Pending') === 'Completed' ? 'completed' : 'pending'}">${v.status || 'Pending'}</span>${csLink}</td>
        <td>
        ...
        <div class="modal-body">
          <form id="violForm">
            <div class="form-group">
              ...
              <input class="form-control" id="violStudentId" placeholder="Enter Student ID..." required ${id ? `value="${viol?.students?.student_id || ''}" readonly` : ''} />
              <small id="violStudentName" style="color:var(--text-muted);margin-top:4px;display:block">${id ? `✅ ${viol?.students?.full_name || ''}` : ''}</small>
            </div>
            <div class="form-group">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input class="form-control" id="violSHB" value="${viol?.shb_article_section || 'Article 8 Section 3'}" placeholder="e.g. Article 8 Section 3" />
              </div>
              <div class="form-group">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    <label>Status</label>
                    <select class="form-control" id="violStatus">
                        <option value="Pending" ${viol?.status === 'Pending' ? 'selected' : ''}>Pending</option>
                        <option value="Completed" ${viol?.status === 'Completed' ? 'selected' : ''}>Completed</option>
                    </select>
                </div>
            </div>
